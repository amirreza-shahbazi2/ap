...
public class Main_EX2_PM_3_1_D {
    public class PacmanGUI extends JFrame implements KeyListener {
        ...
        private void drawScore(Graphics2D g2d) {
            ...
            long remainingTime = (gametime - (System.currentTimeMillis() - startTime))/1000;
            g2d.drawString("time:"+ remainingTime+"s", 25, 75);
        }

        private void logic() {
            if (System.currentTimeMillis() - startTime > gametime){
                System.out.println("time ended. final score: "+score);
                System.exit(0);
            }
            if (score>=maxScore){
        ...
        private void handleCrossBorder() {
            ...
            if (pacmanPoint.x >= width / boxSize) {
                pacmanPoint.x = (width / boxSize) - 1;
            }
            if (pacmanPoint.y < 0) {
                pacmanPoint.y = 0;
            }
            if (pacmanPoint.y >= height / boxSize) {
